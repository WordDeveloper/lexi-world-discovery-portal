...
export function render() {
  ...
  const preview = el("div.center", {}, [avatarArt(p.avatar, 130)]);
  ...
  AVATAR_COLORS.forEach((c) => {
    ...
    sw.addEventListener("click", () => { p.avatar.color = c; save(); preview.innerHTML = ""; preview.appendChild(avatarArt(p.avatar, 130)); [...colours.children].forEach((x) => x.style.border = "3px solid transparent"); sw.style.border = "3px solid var(--brand)"; });
  ...
  const story = el("div.card.pad-sm", { style: { textAlign: "center" } }, [
    ...
    el("div", { style: { margin: "8px auto" } }, [storylingArt(p.storyling.species, p.storyling.stage, 100)]),
